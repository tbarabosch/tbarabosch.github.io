...
catalog_tags_file="${temporary_dir}/catalog-tags.txt"
...
if [[ -s "${unique_rows_file}" ]]; then
  cut -f1 "${unique_rows_file}" |
    LC_ALL=C sort -u |
    LC_ALL=C sort -f >"${catalog_tags_file}"
else
  : >"${catalog_tags_file}"
fi

{
  ...
  echo '# Every tag used by a post is listed, including singletons.'
  while IFS= read -r tag; do
    escaped_tag="${tag//\\/\\\\}"
    escaped_tag="${escaped_tag//\"/\\\"}"
    printf -- '- "%s"\n' "${escaped_tag}"
  done <"${catalog_tags_file}"
} >"${generated_file}"
